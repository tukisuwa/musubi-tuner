...
def append_section_idx_to_latent_cache_path(path: str, section_idx: int) -> str:
    """Appends a section index to the latent cache path."""
    body, ext = os.path.splitext(path)
    return f"{body}_{section_idx:04d}{ext}"
...
def encode_and_save_batch(
    ...
    # Get control_indices and target_indices from the first item (assuming all items have the same)
    control_indices = batch[0].control_indices
    target_indices = batch[0].target_indices

    # Apply relative positioning if control_indices and target_indices are provided and enabled
    enable_relative_positioning = dataset.enable_relative_positioning
    logger.info(f"enable_relative_positioning: {enable_relative_positioning}")
    if control_indices is not None and target_indices is not None and enable_relative_positioning:
        base_index = min(control_indices)
        relative_target_indices = [i - base_index for i in target_indices]
        ...
        relative_target_indices = target_indices
    ...
    if one_frame:
        encode_and_save_batch_one_frame(
            vae, feature_extractor, image_encoder, batch, dataset, vanilla_sampling, one_frame_no_2x, one_frame_no_4x
        )
        return
    ...
    # For GroupedImageDirectoryDatasource, item.content can be a list of ndarrays.
    # We need to stack them into a single ndarray for video processing.
    if isinstance(batch[0].content, list):
        # Stack the list of images for each item in the batch to form the frame dimension
        stacked_contents = [np.stack(item.content, axis=0) for item in batch]
        # Stack the items in the batch
        contents = torch.stack([torch.from_numpy(c) for c in stacked_contents])
    else:
        # For other datasources, item.content is already a single ndarray (or should be)
    ...
    if relative_target_indices is not None:
        # multi_frame mode
        latent_f = len(relative_target_indices)
    ...
    if relative_target_indices is not None:
        ...
        for b, item in enumerate(batch):
            ...
            latent = video_lat.squeeze(0)  # C, F, H, W

            # control_latents
            control_latents = []
            if item.control_content is not None:
                for control_image in item.control_content:
                    control_image_tensor = torch.from_numpy(control_image).permute(2, 0, 1).unsqueeze(0).to(vae.device, dtype=vae.dtype) / 127.5 - 1.0
                    control_latent = hunyuan.vae_encode(control_image_tensor, vae).to("cpu").squeeze(0)
                    control_latents.append(control_latent)
            clean_latents = torch.cat(control_latents, dim=2) if control_latents else torch.zeros((16, 0, latents.shape[-2], latents.shape[-1]), dtype=latents.dtype)
        ...
        for b, item in enumerate(batch):
            ...
            for latent_padding in latent_paddings:
                ...
                section_info = {
                    ...
                    "latent_frame_count": latent_f,
                }
                item.latent_cache_path = dataset.get_latent_cache_path(item, section_info)
                save_latent_cache_framepack(
                    item_info=item,
...
def encode_and_save_batch_one_frame(
    ...
    # item.content: target image (H, W, C) or list of target images for multi-frame
    ...
    for b, item in enumerate(batch):
        ...
        clean_latent_indices = relative_control_indices if relative_control_indices is not None else item.fp_1f_clean_indices  # list of indices for clean latents
        ...
        latent_indices = torch.Tensor(relative_target_indices if relative_target_indices is not None else item.fp_1f_target_index).long()  #  can be multiple indices
        ...
        # clean latents preparation (emulating inference)
        num_clean_latents = clean_latent_indices.shape[0]
        clean_latents = latents[b, :, :num_clean_latents]  # C, F, H, W
        if not item.fp_1f_no_post:
        ...
        # Target latents for this section (ground truth)
        target_latents = latents[b, :, num_clean_latents:]  # C, F_target, H, W

        print(f"Saving cache for item {item.item_key} at {item.latent_cache_path}. no_post: {item.fp_1f_no_post}")
        print(f"  Clean latent indices: {clean_latent_indices}, latent indices: {latent_indices}")
        ...
        save_latent_cache_framepack(
            ...
            latent_indices=latent_indices,  # Indices for the ground truth section
...
def encode_datasets_fpack(datasets: list[BaseDataset], encode: callable, args: argparse.Namespace):
    ...
    for i, dataset in enumerate(datasets):
        ...
        for _, batch in tqdm(dataset.retrieve_latent_cache_batches(num_workers)):
            ...
            filtered_batch = []
            for item in batch:
                if hasattr(args, "one_frame") and args.one_frame or item.frame_count is None:
                    # image dataset or one_frame training
                    ...
                    for sec in range(num_sections):
                        # generate path for each section from item info
                        section_info = {
                            "is_vanilla_sampling": args.f1,
                            "section_index": sec,
                            "total_sections": num_sections,
                            "latent_frame_count": latent_f,
                        }
                        item.section_info = section_info
                        p = dataset.get_latent_cache_path(item)
...
def framepack_setup_parser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--remove_old_cache", action="store_true", help="Remove old cache files not in the dataset"
    )
    parser.add_argument(
        "--f1",
        action="store_true",
...
def main():
    ...
    # encoding closure
    # For GroupedImageDirectoryDatasource, we should use one_frame encoding logic
    # because it handles a combination of frames as a single sample.
    # But if multi_frame_training is enabled, use multi-frame logic instead.
    from musubi_tuner.dataset.image_video_dataset import GroupedImageDirectoryDatasource
    is_one_frame_mode = args.one_frame or (isinstance(train_dataset_group.datasets[0].datasource, GroupedImageDirectoryDatasource) and not multi_frame_training_config)
    if is_one_frame_mode and not args.one_frame:
        logger.info("Automatically enabling one-frame encoding logic for GroupedImageDirectoryDatasource.")

    # encoding closure
    def encode(batch: List[ItemInfo], dataset: BaseDataset):
        encode_and_save_batch(
            ...
            args.f1,
            is_one_frame_mode,
            args.one_frame_no_2x,
            args.one_frame_no_4x,
        )

    # reuse core loop from cache_latents with no change
    encode_datasets_fpack(datasets, encode, args)
